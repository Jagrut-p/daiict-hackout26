...
# Backed by PostgreSQL (see database.py / models.py / db_store.py) as of this
# revision. GENERATORS/FACILITIES/SHIPMENTS/VERIFICATIONS/CARBON_RECORDS still
# behave like plain dicts everywhere below — they're ORMDictStore instances
# that persist each read/write to Postgres. If you add a new field, add the
# matching column in models.py too.
# ==============================================================================
...
# PostgreSQL-backed storage. Each of these acts like a plain dict but
# persists through Postgres — see database.py / models.py / db_store.py.
GENERATORS = ORMDictStore(models.GeneratorModel, "id")
FACILITIES = ORMDictStore(models.FacilityModel, "id")
SHIPMENTS = ORMDictStore(models.ShipmentModel, "shipment_id")
VERIFICATIONS = ORMDictStore(models.VerificationModel, "shipmentId")
CARBON_RECORDS = ORMDictStore(models.CarbonRecordModel, "shipment_uuid")
...
async def lifespan(app: FastAPI):
    init_db()
    # Only seed on first run — data now persists in Postgres across restarts,
    # so we don't want to wipe real rows every time the server comes up.
    # Use POST /reset-data if you explicitly want to reload the demo dataset.
    if len(GENERATORS) == 0 and len(FACILITIES) == 0:
        seed_baseline_data()
        print(">>> First run detected: baseline demo data seeded into Postgres. <<<")
    print("\n=======================================================")
    print(">>> EcoSync Backend Online: Connected to PostgreSQL! <<<")
...
def verify_shipment(payload: ShipmentVerifyIn):
    ...
    if payload.shipmentId in SHIPMENTS:
        updated_shipment = dict(SHIPMENTS[payload.shipmentId])
        updated_shipment["status"] = "verified"
        updated_shipment["verifiedWeightTons"] = payload.actualWeightTons
        updated_shipment["receiptId"] = receipt_id
        SHIPMENTS[payload.shipmentId] = updated_shipment
